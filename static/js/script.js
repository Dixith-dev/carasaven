...
function linkify(inputText) {
    ...
    replacedText = inputText.replace(replacePattern1, (match) => {
        ...
        if (match.match(/\.(jpeg|jpg|gif|png)$/) != null) {
            return `<img src="${match}" alt="Image" style="max-width:100%;height:auto;border: 3px solid #ddd;border-radius: 10px;">`;
        } else {
    ...
    //Change email addresses to mailto: links with the word "email".
...
const generateResponse = (chatElement, typingInterval) => {
    const SERVER_URL = "https://carasaven.onrender.com//get-response";
...
const handleSend = () => {
    ...
    setTimeout(() => {
        ...
        const typingInterval = setInterval(() => {
            incomingChatLi.innerHTML = `<img src="static/images/logo.png" alt="Chatbot Icon" class="chat-icon" style="filter: invert(0);height: 40px;width: 40px;"><p>Typing${'.'.repeat(dotCount)}</p>`;
